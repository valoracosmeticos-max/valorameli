    ...
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerencie as lojas conectadas via Mercado Livre OAuth.</p>
        </div>
        <Button asChild>
          <Link to="/setup-lojas"><Settings2 className="h-4 w-4 mr-2" />Setup de Lojas</Link>
        </Button>
      </div>
        ...
        <CardContent className="space-y-3">
          ...
          {!loading && stores.length === 0 && (
            <div className="py-12 text-center space-y-3">
              ...
              <Button asChild>
                <Link to="/setup-lojas">Iniciar Setup de Lojas</Link>
              </Button>
            </div>
          )}
          {stores.map((s) => {
