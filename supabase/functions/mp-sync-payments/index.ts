...
// O refresh_token do ML é de uso único — Sincronizar, Pagamentos MP e o botão de
// renovar podem disparar refresh quase ao mesmo tempo para a mesma loja, e quem
// perde essa corrida recebe invalid_grant mesmo que o outro tenha funcionado.
// Antes de desistir, relê a loja: se alguém já rotacionou o token nesse
// meio-tempo, usa o resultado dele em vez de falhar à toa.
async function refreshIfNeeded(admin: any, store: any): Promise<string> {
  ...
  if (!r.ok) {
    ...
    throw new Error(`Refresh token failed: ${JSON.stringify(j)}`);
  }
